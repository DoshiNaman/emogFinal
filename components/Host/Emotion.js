                ...
                <div className="h-100 grid grid-col-1 grid-flow-col place-items-center ">
                    <div className="h-3/5 w-96 rounded-xl p-3 heading">
                        ...
                        <div className="grid grid-cols-2 text-xl grid-flow-row h-4/5 scl auto-rows-max gap-2 overflow-y-auto">
                            {emotionArray.map((emotion, index) => <div className="py-2 text-left px-3 w-40 h-18 font-bold text-md inputs burlywoodBorder rounded-lg grid relative" key={index} >
                                {emotion.round}
                                <span className="text-base font-normal">{emotion.emotionName}</span>
